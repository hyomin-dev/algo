...
public class Main {
    ...
    static PriorityQueue<Integer> fishPq = new PriorityQueue<>(); // 물고기 크기를 저장하는 우선 순위 큐
    ...
    static void bfs(int[] start){
        ...
        while(!pq.isEmpty()){
            ...
            if(map[node.y][node.x]!=0&&map[node.y][node.x]<size) { // 빈 칸아님 and 해당 칸의 물고기가 아기 상어보다 작은 경우
                ...
                fishPq.poll(); // 현재 크기가 가장 작은 물고기 poll
                fishCount++;
            ...
            if(fishPq.isEmpty()||fishPq.peek()>=size) // 남이 있는 물고기가 없는 경우 종료 OR 남아 있는 물고기의 최소 크기보다 아기 상어크기가 작거나 같은 경우 엄마 상어에게 도움 요청해야할 때
                break;
